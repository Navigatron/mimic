/*
file: markov1
description: Create and use an order-1 markov chain.
...
function merge_sentence(chain, sentence){
	// Clean the sentence:
	// 		Replace scary characters (Don't, makes it more flavorful.)
	//		Remove extra spaces - this is important
	...
	// Clean the array - remove
	// Iterate!
	let pre = "<START>";
	for(let i=0; i<words.length; i++){
		// Increase probability of "pre"=>words[i]
		// What is the current state?
		let current_state = pre;
		...
		// Now, change pre in preparation for the next iteration
		pre = next_state;
	}
	// Now, increase the probability of the last word leading to <END>
	let current_state = pre;
...
function get_next_word(chain, words){
	...
	let current_state = words[words_count-1];
